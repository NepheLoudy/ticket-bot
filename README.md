...
接单不做任何消息轮询/回扫：群内 **@爆米花机-对话型** 并发送「接单」，feishu-gateway 把该消息事件**秒级**转发到本服务（网关路由：含「接单」且 @机器人 → ticket-bot）。
...
> 指定负责人工单「公示即绑定」只写补充负责人与看板人员字段；**状态推进与审批自动通过必须等本人 @机器人 确认**。绑定失败的工单由每分钟对账补偿绑定（只补绑定，不碰状态与审批）。

### 3.5 审批联动（接单 → 机器代理审批通过）
