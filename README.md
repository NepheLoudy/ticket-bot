...
**触发条件**：工单「审批节点」字段变为 **`有组员接单后通过`** 时触发播报（等价于之前的「申请状态=审批中」）。创建与更新事件均支持，通过 `record_id` 去重保证只播一次。
...
每小时检查一次：`审批节点 = 有组员接单后通过` + `当前处理人有值` + 距发起时间超 6 小时。
...
每小时检查一次：`审批节点 = 回执单：是否结单` 且进入「理想结单时间前 1 天」窗口时：
...
| `APPROVAL_NODE_ACCEPT_VALUE` | 触发播报/超时判断的节点值 | `有组员接单后通过` |
...
| `CLOSE_REMINDER_LEAD_DAYS` | 提前 N 天提醒 | `1` |
