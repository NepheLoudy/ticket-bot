...
2. 机器人匹配该群最新待接单工单（指定负责人工单只匹配本人），执行：
   - 项目状态 `waiting → in_progress`（看板无对应记录时仅告警跳过，**不阻断接单**——搬运是 category 门控，看板是派生视图）；
   - 群内回执卡片「✅ 某某 已确认接单」。
...
每小时检查一次：`审批节点` 处于任一触发节点（拆段匹配）+ `当前处理人有值` + `补充负责人为空`（播报即绑定成功的指定负责人工单天然跳过）+ 距发起时间超 6 小时。

- **第 2 轮起**：私聊面向组别对应组长（无人接单升级，同单间隔 ≥3 小时）；
- **当前处理人 == 发起人**（多半还没人接单）：私信该人，按「无人接单」文案提示留意群内响应，或前往审批界面撤回/结单；
- **当前处理人 != 发起人**：
  - 有指定负责人 → 私信当前处理人，引导完成接单确认；
  - 无指定负责人 → 重走公开问询流程（强调还没人接单，@组长）；**每 6 小时一次、每单封顶 2 次**（至少一群发送成功才占用额度；封顶后由组长私聊承担持续提醒）。

### 5. 结单提醒（理想结单时间过后）

每小时检查一次：`审批节点` 处于「回执单：是否结单」（拆段匹配，多组别拼接值兼容）且已过「理想结单时间后 1 天」时，由机器人本人（应用机器人，非 webhook）**私聊当前处理人一次**（已私聊过不重复）。无转群兜底（用户裁定 2026-09-05）；未结单工单的持续曝光由 pm-robot 每日 DDL 播报的「工单结单」分栏承担。
...
| 变量 | 说明 | 默认 |
|------|------|------|
| `QUIET_HOURS_START` / `QUIET_HOURS_END` | 播报静默窗口 `[START, END)`（Asia/Shanghai 整点，支持跨午夜写法） | `2` / `9` |
...
| GET | `/api/tickets/unclosed-by-group` | 未结单工单按负责人组别分桶（供 pm-robot DDL 分栏取数；含 🆘无人接单 `unclaimed` 桶） |
...
│   ├── utils/quietHours.js        # 晚间静默闸门（积压持久化/冲刷）
│   ├── config.js                  # 配置中心
