...
2. 机器人按「接单排队」匹配工单（见 3.3），执行：
   - 项目状态 `waiting → in_progress`（看板无对应记录时仅告警跳过，**不阻断接单**——搬运是 category 门控，看板是派生视图）；
   - 群内回执卡片「✅ 某某 已确认接单」；
3. **多人单**（「是否允许多人接单」=是，详见 LOGIC-MAP §1.3）确认后**不即时通过审批**：写「多人接单截止」开启续接窗口、到期由对账自动通过；截止写入失败时降级为「接单即自动通过」（窗口不成立，绝不卡单）。

> 指定负责人工单「公示即绑定」只写补充负责人与看板人员字段；**状态推进与审批自动通过必须等本人确认**。绑定失败的工单由每分钟对账补偿绑定（只补绑定，不碰状态与审批）。绑定写入的补充负责人 ≠ 已确认——本人来确认是正常流程动作（公示群 @机器人 或私聊机器人发「接单」，短窗内重复消息会被忽略）。

### 3.3 接单排队（同群多张待接单工单的序号区分）

一个群内同时存在多张「等待接单」的工单时（没人接的单、多人单等待续接、指定负责人待本人确认），固定发「接单」无法表达接的是哪张（撞听）。规则：

- 群内**只有一张**待接单工单 → 提示词为「接单」，行为与以前完全一致；
- 群内**复数张** → 各工单卡片提示「接单1」「接单2」…（**最新播报的为「接单1」**），组员按想接的单发对应序号词（兼容「确认接单N」）；此时发裸「接单」会收到提示（列出现有数量与序号范围），不会错接；
- **跟排队一样**：有人接单（含指定负责人确认）或多人单窗口结束、工单离开队列后，剩余工单序号前移，仅剩一张时回落「接单」——机器人自动改写此前发出的工单卡片里的接单提示行（IM 卡片更新接口；webhook 兜底发出的卡片无 message_id 不可改写，只对新卡生效）；
- **多人单窗口到期即出队**：截止时间已过的多人单不再出现在接单队列/卡片提示里（对账会在 1 分钟内关闭窗口并自动通过审批）；
- 序号**不落库**：按源表数据（创建时间倒序）实时推导，跨重启稳定一致；卡片登记为内存态，重启后已发旧卡不再改写，新播报自然按最新队列带序号；
- 面向多组别的工单按群独立编号，只有复数工单并存的群才启用序号词；同一工单在多群可分别带不同序号。
...
| `指定负责人` | —（v65 起废止专项搬运） | 指定负责人公示时即写入「补充负责人」，看板人员由补充负责人全员统驱 |
...
| `MULTI_ACCEPT_WINDOW_FIELD` | 窗口截止写回的源表字段（文本型，值为带 +08:00 的 ISO 文本；数字毫秒会被飞书拒收 TextFieldConvFail） | `多人接单截止` |
...
|------|------|------|
| GET | `/api/health` | 健康检查 |
| GET | `/api/tickets/policy` | 定制窗口：播报路由/审批节点/多人单窗口/组长映射/门禁等定制项全景（只读） |
